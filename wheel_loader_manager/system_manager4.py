...
# The New Memory Driving Node
from wheel_loader_manager.behaviours.reusable_nav_goal import ReusableNavGoal

class SystemManager4(Node):
    def __init__(self):
        super().__init__('system_manager4')
        ...
        # 2. Memory Nodes (Listens to the standard RViz button)
        drive_to_pile = ReusableNavGoal(name="DriveToPile", node=self, topic_name='/goal_pose')
        drive_to_dump = ReusableNavGoal(name="DriveToDump", node=self, topic_name='/goal_pose')

        # 3. Vision & Point Cloud Phase
        enable_tracker = EnableTracker(self)
        align = AlignWithPile(self)
        disable_tracker = DisableTracker(self)
        
        # 🚀 THE BLIND SPOT FIX: Creep forward 35cm before scanning!
        creep_forward = HatchOpenReverseBlindDrive(
            name="CreepForward", node=self, distance=0.35) 
            
        start_rock = StartRocking(self)
        wait_scan = WaitForScanTime(20.0)
        save_cloud = SaveCloud(self)
        process_cloud = ProcessCloud(self) # Fails and stops the loop when the pile is empty
        
        # 4. Standoff & Scoop Phase
        ...
        scoop_dump_cycle.add_children([
            # Go to Pile & Scan
            drive_to_pile,
            enable_tracker, align, disable_tracker, creep_forward,
            ...
            # Dump & Reset
            drive_to_dump,
            open_hatch, wait_open, drive_back_dump, close_hatch, wait_close, nav_mode
        ])

        # 2. The Master Tree (No fake decorator needed!)
        master_sequence = py_trees.composites.Sequence("MasterSequence", memory=True)
        master_sequence.add_children([
            wait_loc,
            scoop_dump_cycle
        ])
    ...
    def tick_tree(self):
        ...
        if self.tree.root.status == py_trees.common.Status.SUCCESS:
            self.get_logger().info("=== CYCLE COMPLETE! RESETTING FOR NEXT LOOP ===")
            # This instantly resets the entire tree back to a blank slate!
            ...
            # The tree will naturally rest in the FAILURE state and stop moving.
